...
```mermaid
flowchart LR
  A[Connected sources\nDrive - Email - Slack - Chat logs] --> B[Structural parsing\n(no heavy reasoning)]
  B --> C[ThoughtUnits\n(ts, author, thread, uri, type)]
  C --> D[Cheap embeddings]
  D --> E[Semantic clustering]
  E --> F[Reasoning skeleton\n(form only)]
  F --> G[Conservative heuristics\nstability/drift flags]
