---
title: "SPL architecture"
description: "Smart Persistence Layer architecture across GraphRAG, DBB/DS, cache, and LoRA lifecycle."
---

# Smart Persistence Layer (SPL) - Architecture Specification

## Overview

This document provides a production-grade description of the Smart Persistence Layer (SPL) in Membria: how GraphRAG, memory, DBB/DS, knowledge cache, and the LoRA lifecycle work together.

---

## 1. What is SPL in Membria

**SPL (Smart Persistence Layer)** is the layer between the user (or team) and any AI models that makes intelligence **cumulative** rather than disposable.

It solves one fundamental problem:

> In conventional AI, intelligence "dissolves" in chats - many conversations, little persistent structure. SPL transforms the stream of interactions into stable, reusable memory with causal relationships, provable provenance, and governance capability.
...
## 2. Core SPL Subsystems

SPL is implemented as a set of interconnected subsystems:
...
Core SPL principle: store structure, not all words:
...
SPL relies on a normalized event stream from external systems (email, DMS, billing, identity, etc.).
...
In SPL, decision irreversibility is not determined by LLM and not derived from text. It is established through external Commitment Events - facts of real-world change that cannot or should not be undone without trace.

SPL fundamentally distinguishes:
- Discussion
- Intent
- Decision
- Irreversible action
...
Commitment Events enter SPL through the event logger and are treated as facts, not interpretations.

#### Decision Window

A decision in SPL is not treated as a message, but as a Decision Window - a temporal window of events within which an irreversible action occurred.
...
Irreversibility in SPL is achieved not through LLM, but through:
```
...
Every decision in SPL has a stable decision_id that persists throughout the decision's lifecycle.
...
Every decision in SPL has an explicitly defined scope - the boundaries of decision applicability.
...
SPL allows existence of conflicting decisions.

When a conflict is detected, SPL:
- Records the conflict fact
- Indicates affected decision_ids
- Links conflict to policy or responsible person

SPL does not automatically choose the correct decision. Conflict resolution is always a political or human action, recorded as an event.

### 4.7 Human override and automation boundaries

SPL allows manual intervention by authorized users.
...
GraphRAG in SPL: first filter permitted graph area, then retrieve relevant subgraphs, then apply semantics.
...
### 5.4 Retrieval Orchestrator as SPL core (budget-aware GraphRAG)
...
## 6. SPL Memory: What Membria Remembers

SPL memory is multi-layered:
...
Because SPL holds long-term memory, meaning:
- Lots of data
- Personal context
- Privacy is critical
- Latency matters
...
- SPL understands: decision was successful or failed
...
- SPL compared: where did local model err? why?
...
5. On new query, SPL first searches graph and cache, then provides context to local model
...
## 10. How SPL Protects Against Input Garbage
...
SPL is the layer that transforms disposable AI conversations into persistent institutional memory.
...
- Human-in-the-loop - SPL never decides, only supports decisions
