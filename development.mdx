---
title: 'Client layer: DBB & Decision Surface'
description: 'How the client captures decisions and surfaces signals without backend escalation'
---

<Info>
  The client layer includes the Decision Black Box (DBB) and the Decision Surface (DS). DBB captures decision pivots locally; DS renders signals from immutable decision records.
</Info>

<Steps>
<Step title="Capture decision moments">
DBB watches interaction streams (chat, docs, notes). It detects commitment phrases, extracts context and alternatives, and freezes reasoning before outcomes contaminate memory.
</Step>
<Step title="Create immutable decision records">
Each decision record includes assumptions, alternatives, confidence, and predictions. The system automatically computes **Value of Information (VoI)** and maintains a **POMDP belief state** for every record.
</Step>
<Step title="Render signals in the Decision Surface">
DS presents VoI-ranked open loops, hidden disagreement (POMDP alerts), drift, and track record signals. It does not read raw chat, only DBB outputs.
</Step>
</Steps>

## Minimal UI touches

- Inline capture cues near messages that became decision candidates.
- Decision Surface cards with signal summaries.
- A low-friction panel for confirm/edit/reject.

### Signal Extraction Quality

Each extracted signal includes:
- `signal_type`: AGREEMENT | SILENCE | SKEPTICAL | ACTION
- `confidence`: 0.0-1.0 (LLM extraction confidence)
- `cultural_modifier`: high_context | low_context
- `behavioral_confirmation`: bool (did action follow words?)

**Rules:**
- **Confidence < 0.7** → Flag for human review.
- **SILENCE** → Require `behavioral_confirmation` within 48h.
- **Cultural modifier** is applied based on team settings (e.g., silence in High Context != agreement).

## Client vs backend

| Client layer (DBB/DS) | Backend layer (KCG/DoD/Gateways) |
| --- | --- |
| Captures decisions and reasoning locally | Escalates to external LLM council when needed |
| Immutable decision records | Immutable knowledge payloads (Arweave) |
| Decision Surface signals | Knowledge Cache Graph + gateway consensus |

When a decision requires external knowledge or verification, the client sends a DoD request to the backend. The backend writes a knowledge event and returns a verified answer that the client can store or reference.
