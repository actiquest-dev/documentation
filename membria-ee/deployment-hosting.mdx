...
# Deployment and hosting (Enterprise Edition)

## Membria Enterprise Copilot - Technical architecture (on-premise)

**Audience:** IT, security teams

## Summary in two sentences

Membria is a closed AI platform deployed inside a bank data center. It does not send data to the public cloud and automates up to 80 percent of operational workload (AML, fraud, documents, support).

It runs a fast XLAM model (under 1 second) for 95 percent of tasks and escalates to Qwen 235B for complex cases.
...
| --- | --- | --- |
| Router | Classifies the request (FAQ, analysis, action), selects the domain (AML, Fraud, Legal), checks security (DLP, RBAC, PII) | <= 50 ms |
| Planner | Breaks tasks into steps, selects tools (graph query, vector search, API), controls XLAM or Qwen routing, validates policy | <= 200 ms |
...
| Latency | Total retrieval <= 300 ms p95 |
...
| Query latency p95 | <= 1000 ms (all types) |
...
| --- | --- | --- |
| Real-time copilot | REST sync | <= 1.2 sec |
| Batch scoring | ETL nightly | 1-2 hours |
| Streaming fraud | Kafka or event stream | <= 100 ms |
| Legacy query | REST or SOAP via ESB | <= 2 sec |
...
# Membria Enterprise AI Hub - presentation outline

## Slide 1: Title

Membria Enterprise AI Hub

Closed AI platform for banks

On-premise. No data in the cloud.

On-prem AI, graph analytics, agents, and LLM orchestration. Runs anywhere: Russia, Kazakhstan, Europe, Asia.
...
What is changing in banks? The perfect storm:
...
Bottom line: banks need AI inside their data center.

## Slide 3: Solution

Membria: AI hub inside your data center.
...
Cost: 250K USD equipment plus 180K USD per year operations
Payback: 1 year (staff savings)
...
## Slide 6: Request flow (AML UBO tracing)

(Diagram placeholder)

## Slide 7: Component 1 - Router + Planner

| Component | Functions | SLA |
| --- | --- | --- |
| Router | Classifies task (FAQ, analysis, action), selects domain (AML, Fraud, Legal), checks security (DLP, RBAC), chooses XLAM or Qwen | <= 50 ms |
| Planner | Breaks into steps, selects tools (graph, vector, API), builds context, controls XLAM or Qwen, checks compliance | <= 200 ms |

## Slide 8: Component 2 - XLAM Large
