...
## Team-level Decision Intelligence (DI)

SMB workspaces include the **DI Engine**, which optimizes team attention across many projects.

- **Value of Information (VoI):** Automatically ranks the shared "Decision Surface" so the team knows which open loop needs closing first based on potential project impact.
- **Hidden Divergence Detection (POMDP):** Tracks the belief states of multiple team members. It can detect when a group has reached "Surface Agreement" (e.g., everyone says "Yes") but underlying signals (tone, conflicting assumptions) suggest a high probability of a hidden disagreement.
- **Priority Queue Sequencing:** Optimizes the order of team decisions using a transparent Value/Urgency score to break bottlenecks. Unlike opaque algorithms, this is predictable and explainable.

---

## Cognitive Safety for Professional Teams

Trust is the currency of SMBs (agencies, legal, audit). Membria protects that trust with a **Cognitive Safety Layer**:

- **Bias Firewall:** Flags common logic errors like "Sunk Cost Fallacy" in drafts before they go to clients.
- **Resonance Alerts:** If the AI (or a team member) agrees too quickly with a risky premise, the system flags a "Low Friction Alert" to encourage double-checking.
- **Debiasing:** Prompts teams to consider alternatives when "Anchoring" effect is detected.
