...
5. **Finalize**: validation event closes the Intent; knowledge becomes queryable. The gateway returns the result plus **DI Signals** (VoI/POMDP state shifts) to the client.
...
The backend graph is the source of truth for knowledge updates, council escalations, and immutable storage. Client captures decisions; backend validates and stores knowledge. They are separate by design but interlock through DoD to feed the client's **Decision Intelligence Engine**.
