...
> It can run as **Membria Cloud (managed)** today and later as **Membria Self-Hosted (local/on-device)**, while using the same core
> principles: **local-first reasoning where possible, explicit memory, explainable retrieval, and selective escalation**.
...
```mermaid
flowchart LR
  U[User] --> UI["Membria Client UI (Chat - Decision Surface - Search)"]
  UI --> ORCH[Router / Orchestrator]

  ORCH --> SLM["Local SLM Runtime (optional now, default later)"]
  ORCH --> MEM["Local Memory Layer (SQLite + DuckDB)"]
  ORCH --> GR["Local GraphRAG (Temporal Ontology + Vectors)"]
  ORCH --> DBB["Decision Black Box (decision extraction agent)"]
  ORCH --> DI["Decision Intelligence Engine (VoI + POMDP)"]

  ORCH -->|Escalate only if needed| COUNCIL["LLM Council (10 models)"]
  COUNCIL --> KCG["Global Knowledge Cache / KCG (decentralized)"]
  COUNCIL --> CUR["Knowledge Curator (verification + fusion)"]
  CUR -->|Updates| GR
  CUR -->|CAG / cache entries| MEM
  CUR -->|DI signals| DI
  CUR -->|Optional training artifacts| SF["SkillForge (LoRA generation)"]
  SF --> SLM
```
...
- **Open loops (VoI-ranked)**: Unresolved threads prioritized by Value of Information.
- **Hidden disagreements**: POMDP signals detecting where surface agreement masks real skepticism.
- **Assumption drift**: Beliefs that changed across time/sources.
- **Precedents**: Similar past decisions and their outcomes.

DS reads from the **Decision Intelligence Engine + memory graph**, not from raw chat scrollback.
...
## 6) Decision Intelligence (DI) Engine

The DI engine runs on top of the DBB and memory graph to optimize joint human-AI deliberation.

- **Value of Information (VoI):** Ranks open loops by the expected utility of resolvement. It tells you *which* decision needs your attention first based on potential impact.
- **POMDP Belief Tracking:** Maintains a probabilistic model of the "true" consensus state. It flags "Hidden Disagreement" when explicit agreement (e.g., "+1") is accompanied by skeptical metadata (latency, conflicting evidence, red-teaming cues).
- **Decision Sequencing:** Recommends the optimal order of decisions to minimize project duration or risk, identifying "bottleneck" choices before they block parallel work.

---

## 7) Orchestrator: the runtime brain
...
## 8) Local Memory Layer (Hybrid + Hierarchical)
...
## 9) Local GraphRAG and "ThoughtUnits"
...
## 10) When local AI is not enough: Council + Global Cache
...
## 11) SkillForge and LoRA patches (Selective, domain-shaped)
