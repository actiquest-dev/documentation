...
- Providing signal data for **Decision Intelligence** (VoI scoring and POMDP updates)
...
- **DI Signals:** Latency, skeptical cues, redundancy markers, and confidence deltas for belief state updates.
...
- DS reflects:
  - **VoI-ranked Open loops**
  - **Hidden Disagreement (POMDP)**
  - Drift
  - Unresolved assumptions
  - Outcome deltas

DBB provides the raw structural truth; the **Decision Intelligence Engine** provides the prioritization and belief-state overlays.
...
## 8. Integration with Decision Intelligence (DI) Engine

DBB is the primary sensor for the DI Engine.

### 8.1 VoI Scoring Loop
DBB extracts the "stakes" and "missing data" from the stream. The DI Engine uses these to compute **Value of Information (VoI)**, which DS uses to rank open loops.

### 8.2 POMDP Belief Update
DBB captures sub-lingual metadata:
- Response latency between team members
- Skeptical questioning patterns
- Red-teaming cues
These are emitted as signals to the DI Engine's **POMDP (Partially Observable Markov Decision Process)** tracker to detect "Surface Agreement."

---

## 9. LoRA Router Policy
...
## 11. LoRA Ownership and Governance Model
...
## 13. Routing Policy (Default)

### 13.1 Normal Path (approx 80-90%)
...
## 19. Council Cost and Escalation Governance
