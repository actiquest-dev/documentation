...
- A **Decision Surface (DS)** that reads DBB + memory (not raw chat) to show what matters, ranked by **Value of Information (VoI)**.
- A **Decision Intelligence (DI) Engine** that tracks **POMDP belief states** to detect hidden disagreement and optimizes decision sequencing.
...
- **DI Engine:** Decision Intelligence Engine; optimizes human-AI collaboration through VoI scoring, POMDP state tracking, and sequencing.
- **VoI:** Value of Information; the expected utility gain from acquiring more data before deciding.
- **POMDP:** Partially Observable Markov Decision Process; used to track the probabilistic state of team consensus.
...
- **Decision Intelligence (DI) Engine**: VoI scoring, POMDP belief updates, sequencing optimization.
...
DBB stores structure; raw logs can stay in source systems, referenced via URIs. It emits signals (latency, confidence, red-teaming cues) to the DI Engine for belief state updates.

---

## 12. Decision Intelligence (DI) Engine

### 12.1 Mission
The DI Engine optimizes the decision-making loop by processing DBB signals and graph data.

### 12.2 Capabilities
- **VoI Scoring:** Dynamically computes the Value of Information for every "Open Loop". If a decision is high-stakes but low-data, its VoI rises, bringing it to the top of the DS.
- **POMDP Belief Tracking:** Models the consensus as a hidden state. It uses Bayesian updates from interaction signals to detect "Surface Agreement" (high explicit agreement, low underlying consensus confidence).
- **MCTS Sequencing:** Explores the tree of potential decisions to recommend orderings that minimize the project's critical path.

---

## 13. Decision Surface (DS)

### 13.1 Why DS is the home screen
...
### 13.2 DS reads DBB, not chat
DS is built from:
- **VoI-ranked Open loops** (unresolved threads prioritized by potential utility)
- **Hidden Disagreement alerts** (POMDP-derived warnings of low consensus)
...
### 13.3 DS cards (safer, less scary semantics)
...
## 14. UI behaviors relevant to runtime
...
### 14.2 Source handling (files, links, snippets)
...
### 15.1 Required telemetry (privacy-safe)
...
### 15.2 Debug views for power users
...
## 18. Roadmap (runtime-facing)

1. Cloud MVP: Chat + Sources + explainable retrieval + caching.
2. DBB MVP: decision extraction from chat + minimal DS.
3. Decision Intelligence Phase: VoI ranking and POMDP belief tracking.
4. Council + Curator + KCG caching loop.
