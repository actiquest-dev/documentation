...
---
> Membria CE uses a decentralized backend to verify, store, and share knowledge artifacts that exceed local confidence. This backend is
> separate from the client runtime and is invoked only when escalation is required.
...
graph TB
    ...
    KnowledgeCreator -->|"New Answer + DI Signals"| Router
...
A validation event is recorded on Peaq with final status and confidence score. This result is returned to the client along with **DI Signals** (e.g., belief-state delta for the POMDP tracker) to update the Decision Surface.
