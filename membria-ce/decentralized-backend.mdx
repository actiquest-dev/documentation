...
graph TB
  subgraph Client["Client layer"]
    SLM["User or SLM"]
    DoDAgent["DoD agent (lightweight client)"]
    SLM <--> DoDAgent
  end
  ...
  subgraph Gateway["Gateway layer - knowledge lifecycle engine"]
    ...
    subgraph FastPath["Fast path"]
      ...
      SemanticSearch["Semantic search (graph transformer, cuGraph)"]
    end

    subgraph SlowPath["Slow path"]
      LLMEscalator["LLM escalator"]
      KnowledgeCreator["Knowledge creator (payload, Arweave, consensus)"]
    end

    subgraph Background["Background"]
      EventProcessor["Event processor"]
      ArchiveEngine["Archive engine (batching, DuckDB, Parquet)"]
    end
  end

  subgraph Teacher["Teacher services"]
    LLMX["Teacher LLM (OpenRouter)"]
  end

  subgraph Blockchain["Peaq blockchain"]
    PeaqSubgraph["Peaq subgraph (pallet-kcg)"]
    PeaqConsensus["Consensus engine (verification logic)"]
  end

  subgraph Storage["Permanent storage (Arweave)"]
    JSONFiles["JSON answers (answers/{id}.json)"]
    ParquetArchives["Parquet archives (batches/{ts}.parquet)"]
    ManifestIndex["Manifest index (archive metadata)"]
  end
